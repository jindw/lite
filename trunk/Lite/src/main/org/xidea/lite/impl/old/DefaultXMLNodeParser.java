...
public class DefaultXMLNodeParser implements NodeParser<Node> {
	...
	private void parseCDATA(Node node, ParseContext context) {
		...
		try {
			...
			context.appendAll(context.parseText(
					((CDATASection) node).getData(), Template.EL_TYPE));
			context.append("]]>");
		} finally {
			if (needFormat) {
	...
	private void parseTextNode(Node node, ParseContext context) {
		...
		if (text.length() > 0) {
			...
			try {
				context.appendAll(context.parseText(text,
	...
	private void parseAttribute(Node node, ParseContext context) {
		...
		if (ParseUtil.isCoreNS("xmlns:c".equals(name) ? "c" : attr.getPrefix(),
				value)) {
			return;
		}
