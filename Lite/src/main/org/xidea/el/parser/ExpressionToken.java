...
public abstract interface ExpressionToken {
	...
	public static final int OP_SUB = 1<<5 | 5<<1 |1;//-;
	

	//五：
	public static final int OP_LT =    0<<5 | 4<<1 |1;//<;
	public static final int OP_GT =    1<<5 | 4<<1 |1;//>;
	public static final int OP_LTEQ =  2<<5 | 4<<1 |1;//<=;
	public static final int OP_GTEQ =  3<<5 | 4<<1 |1;//>=;
	public static final int OP_EQ =    4<<5 | 4<<1 |1;//==;
	public static final int OP_NOTEQ = 5<<5 | 4<<1 |1;//!=;
	
	//四:
	public static final int OP_AND = 0<<5 | 3<<1 |1;//&&;
	public static final int OP_OR =  1<<5 | 3<<1 |1;//||;
	
	//三：
	public static final int OP_QUESTION =        0<<5 | 2<<1 |1;//?;
	public static final int OP_QUESTION_SELECT = 1<<5 | 2<<1 |1;//:;

	//二：
	//与Map Join 共享字面量（map join 会忽略）
	public static final int OP_PARAM_JOIN = 0<<5 | 1<<1 |1;//,

	//一：
	//与三元运算符共享字面值
	public static final int OP_MAP_PUSH = 0<<5 | 0<<1 |1;//:,
