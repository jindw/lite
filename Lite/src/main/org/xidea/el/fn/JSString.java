...
public class JSString extends JSObject implements Invocable {
	...
	// 15.5.4.8 String.prototype.lastIndexOf(searchString, position)
	public int lastIndexOf(String thiz, String sub,int pos) {
		return thiz.lastIndexOf(sub,pos);
	}
	// 15.5.4.11 String.prototype.replace(searchValue, replaceValue)
	public String replace(String thiz, String searchValue,String replaceValue) {
		return thiz.replace(searchValue,replaceValue);
	}
	// 15.5.4.13 String.prototype.slice(start, end)
	public String slice(String thiz, Object[] args) {
		int size = thiz.length();
		int begin = JSArray.toSliceRange(ECMA262Impl.getIntArg(args, 0, 0),size);
		int end = JSArray.toSliceRange(ECMA262Impl.getIntArg(args, 1, size),size);
		if(begin<end){
